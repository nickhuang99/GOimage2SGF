...
//              << g_device_path;
//     if (!captureFrame(g_device_path, frame)) {
//       THROWGEMERROR("MinMax Test FAILED: Could not capture frame from
//       webcam.");
//     }
//   } else {
...
//     frame = cv::imread(imagePath);
//     if (frame.empty())
//       THROWGEMERROR("MinMax Test FAILED: Failed to load image: " +
//       imagePath);
//   }

//   LOG_INFO << "Image loaded/captured successfully. Size: " << frame.cols <<
//   "x"
//            << frame.rows;
...
//   } else {
//     LOG_ERROR
//         << "Positional detection FAILED. Could not find all four
//         candidates.";
...
void runDetectBoardWorkflow(const std::string &imagePath) {
  ...
  if (refine_board_corners_pass2(frame, p1_candidates, refined_corners)) {
    ...
    cv::Mat display_image = frame.clone();
    cv::polylines(display_image, {refined_corners}, true, {0, 255, 0}, 2,
                  cv::LINE_AA);
    for (const auto &p : refined_corners) {
      cv::circle(display_image, p, 7, {0, 0, 255}, -1);
    }
    cv::putText(display_image, "Pass 2 Refined Result", {15, 25},
    ...
    for (const auto &cand : p1_candidates) {
      cv::polylines(display_image, {cand}, true,
                    {(idx * 80) % 255 * 1.0f, 200, 255}, 1);
      idx++;
    }
...
//     LOG_INFO << "Board detection successful. Found " <<
//     board_candidates.size()
//              << " candidate(s).";
...
//       cv::Scalar color((cand_idx * 80 + 50) % 255, (cand_idx * 50 + 150) %
//       255,
//                        (cand_idx * 120) % 255);
//       if (corners.size() == 4) {
...
//     cv::imshow("Rough Board Detection Result (All Candidates)",
//     display_image); LOG_INFO << "Displaying results. Press any key to exit.";
//     cv::waitKey(0);
//     cv::destroyAllWindows();

//   } else {
//     LOG_ERROR
//         << "Board detection FAILED. Could not find any suitable
//         quadrilateral.";
