...
bool saveCornerConfig(const std::string &filename, 
                      ...
                      int frame_width, int frame_height,          // Existing, but now explicitly for config
                      const cv::Point2f &tl_raw,
                      const cv::Point2f &tr_raw, 
                      const cv::Point2f &bl_raw,
  ...
  outFile << std::fixed << std::setprecision(1); // For Lab values and percentages
  ...
  outFile << "\n# Sampled Lab Colors from Corrected Image (L:0-255, A:0-255, B:0-255)" << std::endl;
  ...
  outFile.close();
  if (!outFile) { 
    std::cerr << "Error: Failed to properly close config file after writing: " << filename << std::endl;
    return false;
  }
...
void runInteractiveCalibration(int camera_index) {
  ...
  while (true) {
    ...
    if (key_action_type == 27) { // ESC
      ...
      saveCornerConfig(CALIB_CONFIG_PATH, g_device_path, frame_width,
                       frame_height, topLeft_raw, topRight_raw, bottomLeft_raw,
                       bottomRight_raw, sample_data[0], sample_data[1],
