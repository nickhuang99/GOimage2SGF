...
bool captureFrameOpenCV(const std::string &device_path, cv::Mat &frame) {
  ...
  if (!trySetCameraResolution(cap, g_capture_width, g_capture_height, true)) {
    // If strict resolution is required for snapshot, you might throw or log an error.
    // For now, we'll proceed with whatever resolution the camera settled on if trySet fails.
    // The utility function logs the failure in debug mode.
    ...
    if (final_width != g_capture_width || final_height != g_capture_height) {
         std::stringstream ss;
         ss << "Snapshot: Failed to set desired resolution " << g_capture_width << "x" << g_capture_height
            << ". Actual resolution is " << final_width << "x" << final_height << ".";
